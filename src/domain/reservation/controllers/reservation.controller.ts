...
export class ReservationController {
  ...
  async create(
    ...
    return this.reservationService.createReservation(req.user.id, createReservationDto);
  }
  ...
  async checkAvailability(
    ...
    if (location && !validLocations.includes(location as any)) {
      throw new BadRequestException(`Localisation invalide. Doit être l'un des : ${validLocations.join(', ')}`);
    }
    try {
      ...
      throw new BadRequestException('Erreur lors de la vérification de la disponibilité');
    }
  }

  @Get('user/status/:status')
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Obtenir les réservations d\'un utilisateur par statut' })
  ...
  async getUserReservationsByStatus(
    ...
    return this.reservationService.getUserReservations(req.user.id, status as ReservationStatus);
  }
  ...
  @ApiResponse({ status: 200, description: 'Réservation annulée avec succès', type: Reservation })
